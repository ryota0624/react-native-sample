...
export class TopicRepositoryOnMem extends Repository implements TopicReadRepository, TopicWriteRepository {
    ...
    store(topic: Topic, tagIds: TagName[]) {
        ...
        return UserFollowTopicsDao.store(topic.createdUserId.value, topic.id.value);
    }
    ...
    findUserFollows(userId: UserID): Promise<Topic[]> {
        return UserFollowTopicsDao.findByUserId(userId.value).then(topicIdNumbers => {
            return Promise.all(topicIdNumbers.map(topicIdNumber => this.findById(new TopicID(topicIdNumber))))
        });
    }
}
